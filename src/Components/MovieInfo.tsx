...
interface MovieInfoProps {
  ...
  selectedDated: string;
  selectedTime: string;
  seatSelect: boolean;
  ViewSeat: () => void;
}
const MovieInfo: React.FC<MovieInfoProps> = ({ selectedMovie, selectedRegion, selectedLocation, selectedDated, selectedTime, seatSelect, ViewSeat }) => {
  return (
    <div className=" mt-12 border border-gray-400 py-2 w-full bg-black">
      <div className="flex mx-48 text-white">
        ...
        <div>
          <button
            className={`text-xl border rounded-lg py-6 px-2 mt-10 ml-32 ${seatSelect ? "bg-red-500" : "bg-gray-500"
              }`}
            onClick={() => ViewSeat()}
          >
            좌석선택
